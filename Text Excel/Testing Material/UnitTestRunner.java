...
public class UnitTestRunner {
   ...
   /**
    * Checks if input is "test create #" <p>
    * 
    * If the input is of that form, it will redirect output to a file. 
    * When done with the tests, the user should enter, "test end"
    * 
    * @param input The command string that the user entered to be processed.
      ...
      while (file.hasNextLine()) {
         try {
            ...
            if (input.toLowerCase().startsWith("<timeout")) {
               ...
               s = String.format("\t%.1f / %.1f\tTOTAL: %.1f / %.1f\t%s", subSectionPoints, subSectionTotal, points,
                     total, s);
               summary.add(s);
            ...
            if (answerCount == 0 || multiLineMatch(actualFull, expected)) {
               ...
               if (answerCount == 0 && actualFull.length() > 0) {
